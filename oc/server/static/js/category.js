 ...
 * @constructor
 */
oc.Category = function(socket,user) {
...
oc.Category.prototype.goCategory = function(name,id,href) {
...
oc.Category.prototype.createConversations = function(hide,d_list) {
    //id,title,user,date
    var html = '';
...
oc.Category.prototype.showCategoryHead = function(name,id,href) {
...
oc.Category.prototype.goConversation = function(id) {
   var self = this;
   ...
   if (this.currentConversation.data == undefined ||  this.currentConversation.data.conversation.id != id) 
    {
       ...
       $.getJSON('/conversation/'+id,function(data) {
            oc.Nav.hideAll();
            oc.Nav.setTitle(data.conversation.title); 
            ...
            self.socket.addCallback('viewers',function(viewers) {
                ...
                for (v in viewers) {
                    if (!(v in self.currentConversation.viewers)) {
                        var userView = jQuery('<a href="/users/'+v+'" name="'+v+'"><img id="v_'+v+'"src="/static/images/avatars/thumbs/'+viewers[v]+'" /></a>');
                        userView.hide();
            ...
            goog.array.forEach(data.responses,function(r) {
...
oc.Category.prototype.createResponse = function(fadeIn,r_id,r_user,date,content,canVote) {

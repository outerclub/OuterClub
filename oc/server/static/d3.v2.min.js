...
window['leaderboard'] = function(users,nav) {
var w = 875,
    h = 600,
...
var treemap = d3.layout.treemap()
    .size([w, h])
    .sticky(true)
    .value(function(d) { return d.prestige; });
var prestigeBounds = d3.extent(users.children,function(d) { return d.prestige });
var fontScale = d3.scale.linear().domain(prestigeBounds).range([10,50]);
var imageScale = d3.scale.linear().domain(prestigeBounds).range([25,50]);
var paddingScale = d3.scale.linear().domain(prestigeBounds).range([3,15]);

var div = d3.select("#leaderboard").append("div")
...
var cellDiv = div.data([users]).selectAll("div")
      .data(treemap.nodes)
    .enter().append("div")
      .attr("class", "cell")
      .style("background", function(d) {
            var rgb = d3.rgb(color(d.name));
            return "rgba("+rgb.r+","+rgb.g+","+rgb.b+",0.5)"; })
      .style("font-size",function(d) { return fontScale(d.prestige)+"px"; })
      .call(cell);
cellDiv.attr('data-url',function(d) { return "#!/user/"+d.user_id });
var innerDiv = cellDiv.append("div")
        .style("padding","5px");
innerDiv.append("img")
       .attr("src",function(d) { return d.children ? null : "/static/images/avatars/thumbs/"+d.avatar_image; })
       .attr("height",function(d) { return d.children ? null : imageScale(d.prestige)+"px"});
innerDiv.append("span")
       .text(function(d) { return d.children ? null : d.name; });

var boxes = innerDiv.append("div");

boxes.append("div")
            .classed("rank",true)
            .text(function(d) { return d.children ? null : "#"+d.rank })
            .style("padding",function(d) { return paddingScale(d.prestige)+"px"; });
/*
boxes.append("div")
            .classed("prestige",true)
            .text(function(d) { return d.children ? null : d.prestige})
            .style("padding",function(d) { return paddingScale(d.prestige)+"px"; });
            */
